...
]]-- Creditation preserved due to 3rd party script
...
function writescrollCMD(pid, cmd)
    if(cmd[2] ~= nil) then
...
function makecopyCMD(pid, cmd)
    if(getMod(pid) ~= true) then return end
    if(cmd[2] ~= nil) then
...
function init()
	tes3mp.LogMessage(1, "[WRITING]: Writing loaded (through scriptHook).")
end

customEventHooks.registerHandler("OnServerPostInit", init)
customEventHooks.registerHandler("OnGUIAction", GUI)
customCommandHooks.registerCommand("writebook", writebookCMD)
customCommandHooks.registerCommand("writenote", writenoteCMD)
customCommandHooks.registerCommand("writescroll", writescrollCMD)
customCommandHooks.registerCommand("makecopy", makecopyCMD)

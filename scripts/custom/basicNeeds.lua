basicNeedsConfig = require("custom/basicNeedsConfig")
	...
	if logicHandler.CheckPlayerValidity(pid, pid) then
		...
		if listName == oldPlayerName then
			...
			if val < 90 then
				...
				if basicNeedsConfig.needsLogging == true then
				...
				if basicNeedsConfig.needsEmote == true then
					basicNeedsEmote(pid, need)
				end
				if basicNeedsConfig.needsDebuff == true then
...
--Create debuffs specifically for players
	...
	function basicNeeds.startTic(pid)
		...
		if basicNeedsConfig.needsToggle == true and Players[pid].data.debugFlags.haltTracking == false then
	...
	function basicNeeds.hungerTic(pid)
		if Players[pid].data.debugFlags.haltTracking == false then
			...
			if tableHelper.containsValue(activeHungerTimers, listName) ~= true then
				hungerTime = tes3mp.CreateTimerEx("basicNeedsLogic", basicNeedsConfig.needsTimer, "sissss", listName, pid, "hungerTic", "hunger", "hungry", "starving")
	...
	function basicNeeds.thirstTic(pid)
		if Players[pid].data.debugFlags.haltTracking == false then
			...
			if tableHelper.containsValue(activeThirstTimers, listName) ~= true then
				thirstTime = tes3mp.CreateTimerEx("basicNeedsLogic", basicNeedsConfig.needsTimer, "sissss", listName, pid, "thirstTic", "thirst", "thirsty", "dehydrated")
	...
	function basicNeeds.fatigueTic(pid)
		if Players[pid].data.debugFlags.haltTracking == false then
			...
			if tableHelper.containsValue(activeFatigueTimers, listName) ~= true then
				fatigueTime = tes3mp.CreateTimerEx("basicNeedsLogic", basicNeedsConfig.needsTimer, "sissss", listName, pid, "fatigueTic", "fatigue", "tired", "exhausted")
...
--Check if consumed item is food/drink
	function basicNeeds.ingest(pid, itemRefID)
		if tableHelper.containsValue(basicNeedsConfig.foodItems, itemRefID) then
			basicNeeds.ingestApply(pid, "eat", "hunger", "starving", "needs_debuff_00")
		elseif tableHelper.containsValue(basicNeedsConfig.drinkItems, itemRefID) then
...
--Apply consumption to specific need
	...
	function basicNeeds.OnPlayerItemUse(eventStatus, pid, itemRefID)
		basicNeeds.ingest(pid, itemRefID)
	end
	
	function basicNeeds.OnPlayerFinishLogin(eventStatus, pid)
		basicNeeds.startTic(pid)
	end
	
	customEventHooks.registerHandler("OnPlayerFinishLogin", basicNeeds.OnPlayerFinishLogin)
	customEventHooks.registerHandler("OnPlayerItemUse", basicNeeds.OnPlayerItemUse)
	customEventHooks.registerHandler("OnPlayerCellChange", basicNeeds.OnPlayerCellChange)
	customCommandHooks.registerCommand("rest", basicNeeds.rest)

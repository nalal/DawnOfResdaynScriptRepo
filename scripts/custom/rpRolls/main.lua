...
local function doRoll(pid, playerName, rollcheck)
    ...
    if rollCheckID ~= 27 then
    	...
    	tes3mp.SendMessage(pid, color.Cyan ..  "[RP-Rolls]: " .. color.White .. "Invalid skill.", false)
    end
end

function roll(pid, cmd)
...
function loadScript()
	tes3mp.LogMessage(enumerations.log.INFO, "[RP-ROLLS]: RP-Rolls loaded (through scriptHook)")
end

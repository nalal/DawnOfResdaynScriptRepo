...
config.DHMainID = 31360
config.DHPromptId = 31361
...
SCRIPT:AddMethod("AddPermission", function(pname, cell)
	local perms = getPermissionsTable()
...
SCRIPT:AddMethod("RemovePermission", function(pname, cell)
	local perms = getPermissionsTable()
...
SCRIPT:AddMethod("RemoveAllPermissions", function(cell)
	local perms = getPermissionsTable()
...
SCRIPT:AddMethod("RemoveAllPlayerFurnitureInCell", function(pname, cell, returnToOwner)
	local placed = getPlacedTable()
...
SCRIPT:AddMethod("RemoveAllFurnitureInCell", function(cell, returnToOwner)
	local placed = getPlacedTable()
	local cInfo = placed[cell] or {}
...
SCRIPT:AddMethod("TransferOwnership", function(refIndex, cell, playerCurrentName, playerToName, save)
	local placed = getPlacedTable()
...
SCRIPT:AddMethod("TransferAllOwnership", function(cell, playerCurrentName, playerToName, save)
	local placed = getPlacedTable()
...
SCRIPT:AddMethod("GetSellBackPrice", function(value)
	return getSellValue(value)
end)

SCRIPT:AddMethod("GetFurnitureDataByRefId", function(refId)
	return getFurnitureData(refId)
end)

SCRIPT:AddMethod("GetPlacedInCell", function(cell)
	return getPlaced(cell)
end)

SCRIPT:AddHook("OnServerPostInit", "KF_ServerInit", function()
...
SCRIPT:AddHook("ProcessCommand", "KF_Commands", function(pid, cmd)
  if(cmd[1] == "furniture") or (cmd[1] == "furn") then
  	showMainGUI(pid)
  end
end)

SCRIPT:AddHook("OnGUIAction", "KF_GUIAct", function(pid, idGui, data)

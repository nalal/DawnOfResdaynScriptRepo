...
SCRIPT:AddHook("OnGUIAction", "KH_GUIAct", function(pid, idGui, data)
...
SCRIPT:AddHook("OnObjectLock", "KH_ObjectLock", function(pid, cellDescription)
  unlockChecks(cellDescription)
end)

SCRIPT:AddHook("OnContainer", "KH_Container", function(pid, cellDescription)
...
SCRIPT:AddHook("OnObjectDelete", "KH_ObjectDelete", function(pid, cellDescription)
  tes3mp.ReadLastEvent()
	local pname
...
SCRIPT:AddHook("OnPlayerCellChange", "KH_PlyCellChange", function(pid)
  if Players[pid] ~= nil and Players[pid]:IsLoggedIn() then
		local currentCell = tes3mp.GetCell(pid)
...
SCRIPT:AddHook("ProcessCommand", "KH_Commands", function(pid, cmd, message)
